...
public class JHMSortTest {
    ...
    private static List<Long> numList = new ArrayList<>(100000);

    private static List<Long> subNumList;

    static {
        for (int i = 0; i < 10000; i++) {
            numList.add(ThreadLocalRandom.current().nextLong(0, Long.MAX_VALUE));
        }
        subNumList = numList.subList(0, 10000);
    }
    ...
    public void testStringFormat(Blackhole blackhole) {
        List<Long> myList = new ArrayList<>(100000);
        for (int i = 0; i < 100000; i++) {
            myList.add(numList.get(i));
        }
        myList.sort(Comparator.comparing(i -> i));
        blackhole.consume(myList);
    }

    @Warmup(iterations = 5, time = 5)
    // 循环测试不要在方法体中测试
    @BenchmarkMode(Mode.SingleShotTime)
    @Measurement(iterations = 5, time = 5, batchSize = 10000)
    @Benchmark
    public void testString(Blackhole blackhole) {
        List<Long> myList = new ArrayList<>(10000);
        for (int i = 0; i < 10000; i++) {
            myList.add(subNumList.get(i));
        }
        myList.sort(Comparator.comparing(i -> i));
        blackhole.consume(myList);
    }
    ...
    public static void test() throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(JHMSortTest.class.getSimpleName())
